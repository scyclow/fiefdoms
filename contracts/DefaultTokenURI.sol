...
contract DefaultTokenURI {
  ...
  Fiefdoms private immutable kingdom;

  constructor() {
    kingdom = Fiefdoms(msg.sender);
  }

  function tokenURI(uint256 tokenId) external view returns (string memory) {
    // FiefdomArchetype archetype = FiefdomArchetype(msg.sender);


    // bytes memory name = abi.encodePacked('Fiefdom Vassal #', tokenId.toString());
    // address fiefdomAddr = kingdoms.tokenIdToFiefdom(tokenId);
